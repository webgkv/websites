...
// Build blog base URL explicitly (avoid broken `get_url('blog')`).
			...
			if ($news = mysql_select($sqlArticleCat, 'row')) {
				...
				foreach ($abc['languages'] as $i => $v) {
					...
					if (!empty($blog_slug_map[$lang_row_id])) {
						$abc['links'][$lang_url] = array($lang_url, 'blog', $blog_slug_map[$lang_row_id]);
					} else {
						$abc['links'][$lang_url] = array($lang_url, 'blog');
					}
				}
