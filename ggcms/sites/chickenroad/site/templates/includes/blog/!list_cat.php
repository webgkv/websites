<?php
// Legacy category list — same card layout as blog/list.php
...
<section class="py-5">
  <div class="container">
    <div class="row">
<?php }
	$cat_slug = trim((string)($abc['gcats'][$q['category']]['url' . $langid] ?? ''), '/');
	$link = $blog_base . ($cat_slug !== '' ? $cat_slug . '/' : '') . trim((string)($q[$url_key] ?? ''), '/') . '/';
