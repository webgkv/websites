#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Apply Swahili (20) and Lingala (21) to a blog cluster JSON export."""
...
from blog_sw_ln import (  # noqa: E402
...
def apply_cluster(data: dict) -> list[str]:
    ...
    if en_san != (en.get("content") or ""):
        en["content"] = en_san
        logs.append(f"blog#{entity_id} en: sanitized template (h1/alt)")
    ...
    for lang_id, lang in LANG_IDS.items():
        ...
        if not meta or not content:
            logs.append(f"blog#{entity_id} {lang}: translation missing")
            continue
        if plain_len(content) < PLAIN_MIN:
            logs.append(f"blog#{entity_id} {lang}: body too short")
            continue
        ...
        logs.append(
            f"blog#{entity_id} {lang}: body {len(content.encode())}B "
...
def main() -> int:
    if len(sys.argv) < 2:
        print("Usage: apply_blog_sw_ln_cluster.py <cluster.json> [out.json]", file=sys.stderr)
        return 1
    src = Path(sys.argv[1])
