#!/usr/bin/env bash
# Export blog cluster from prod, apply sw/ln, import back.
set -euo pipefail

ENTITY_ID="${1:?usage: sync_blog_sw_ln_prod.sh <entity_id>}"
TOOLS="$(cd "$(dirname "$0")" && pwd)"
DL="${DL:-$HOME/Downloads/02/chickenroad-blog}"
SSH="ssh -i ~/.ssh/webgkv -p 20203 dikodo@38.133.213.49"
REMOTE="cd /home/dikodo/web/chickenroad.run/public_html"
JSON="$DL/seo-blog-${ENTITY_ID}-full.json"
REMOTE_JSON="/tmp/seo-blog-${ENTITY_ID}-full.json"

mkdir -p "$DL"
echo "== Export blog#${ENTITY_ID} =="
$SSH "$REMOTE && php scripts/export_seo_cluster_cli.php blog ${ENTITY_ID} full" > "$JSON"

echo "== Apply sw/ln =="
python3 "$TOOLS/apply_blog_sw_ln_cluster.py" "$JSON"

echo "== Import blog#${ENTITY_ID} =="
scp -i ~/.ssh/webgkv -P 20203 "$JSON" "dikodo@38.133.213.49:${REMOTE_JSON}"
$SSH "$REMOTE && php scripts/import_seo_cluster_cli.php ${REMOTE_JSON} blog ${ENTITY_ID} full"
...
\$id='${ENTITY_ID}';
\$scan=seo_monitor_list_row_issue_scan(\"blog\", \$id);
\$pack=seo_monitor_export_cluster_array(\"blog\", \$id, \"full\");
