#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Load casino_articles sw/ln meta and translation pairs from JSON data files."""
...
DATA_DIR = Path(__file__).resolve().parent / "casino_sw_ln_data"


def _load(entity_id: int) -> dict | None:
    path = DATA_DIR / f"casino_{entity_id}.json"
    if not path.is_file():
        return None
...
def ln_from_fr(entity_id: int) -> bool:
    ...
    if not data:
        return False
    return bool(data.get("ln_from_fr"))
