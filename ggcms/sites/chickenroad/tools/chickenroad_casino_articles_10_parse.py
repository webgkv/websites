# -*- coding: utf-8 -*-
"""Parse casino_articles#10 1win Chicken Road Word HTML into structured body dicts."""
...
def parse_content_to_body(content: str) -> dict:
    ...
    intro_body = by_title.get("Chicken Road on 1win - What Is This Game?", "")
    intro_paras = paras_from_html(intro_body)
    intro_imgs = imgs_from_html(intro_body)

    where_body = by_title.get("Where to Find Chicken Road on 1win", "")
    ...
    play_body = by_title.get("How to Play Chicken Road Alternatives on 1win", "")
    ...
    mobile_body = by_title.get("Chicken Road on Mobile via 1win", "")
    mobile_paras = paras_from_html(mobile_body)
    mobile_imgs = imgs_from_html(mobile_body)

    bonus_paras = paras_from_html(by_title.get("Bonuses and Promos for Chicken Road on 1win", ""))
...
def fix_en_body(body: dict) -> dict:
    out = dict(body)
    out["title"] = "Chicken Road on 1win: alternatives, demo and tips"
    out["description"] = (
        "Chicken Road is not on 1win yet — find Turbo Games alternatives, demo play, "
