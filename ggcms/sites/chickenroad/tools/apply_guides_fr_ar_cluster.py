#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Apply French (3) and Arabic (11) to guides cluster JSON from EN template."""
...
from guides_fr_ar import get_content_override, get_meta, get_pairs  # noqa: E402
...
def apply_cluster(data: dict) -> list[str]:
    ...
    for lang_id, lang in LANG_IDS.items():
        ...
        if plain_len(content) < PLAIN_MIN:
            logs.append(f"guides#{entity_id} {lang}: body too short")
            continue
...
def main() -> int:
    if len(sys.argv) < 2:
        print("Usage: apply_guides_fr_ar_cluster.py <cluster.json> [out.json]", file=sys.stderr)
        return 1
    src = Path(sys.argv[1])
