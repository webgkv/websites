#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Load blog sw/ln meta and translation pairs from JSON data files."""
...
DATA_DIR = Path(__file__).resolve().parent / "blog_sw_ln_data"


def _load(entity_id: int) -> dict | None:
    path = DATA_DIR / f"blog_{entity_id}.json"
    if not path.is_file():
        return None
