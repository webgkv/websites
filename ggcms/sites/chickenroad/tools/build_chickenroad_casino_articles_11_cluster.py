#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Rebuild casino_articles#11 1xBet Chicken Road cluster — clean HTML + locale translations."""
...
from chickenroad_casino_articles_11_locales import IMAGES, LOCALE_META, get_body  # noqa: E402

CLUSTER_IN = Path("/Users/gk/Downloads/05/seo-casino_articles-11-full.json")
OUT_PATH = CLUSTER_IN
...
def build_content(body: dict) -> str:
    ...
    chunks.append(f"<h2>{e(body['h2_lobby'])}</h2>")
    chunks.append(paras(body["lobby_paras"]))
    chunks.append(figure(IMAGES["filter"], body["img_filter_alt"]))
    chunks.append(f"<p><strong>{e(body['quick_check_title'])}</strong></p>")
    chunks.append("<ul>")
    chunks.append(f"<li>{e(body['quick_check_item'])}</li>")
    chunks.append("</ul>")
    chunks.append(paras(body.get("lobby_outro", [])))

    chunks.append(f"<h2>{e(body['h2_play'])}</h2>")
    chunks.append(paras(body["play_paras"][:2]))
    chunks.append(figure(IMAGES["play"], body["img_play_alt"]))
    chunks.append(paras(body["play_paras"][2:]))

    chunks.append(f"<h2>{e(body['h2_why'])}</h2>")
    chunks.append(paras(body["why_paras"][:3]))
    chunks.append(figure(IMAGES["versions"], body["img_versions_alt"]))
    chunks.append(paras(body["why_paras"][3:]))

    chunks.append(f"<h2>{e(body['h2_mobile'])}</h2>")
    chunks.append(paras(body["mobile_paras"][:2]))
    chunks.append(figure(IMAGES["mobile"], body["img_mobile_alt"]))
    chunks.append(paras(body["mobile_paras"][2:]))

    chunks.append(f"<h2>{e(body['h2_bonuses'])}</h2>")
    chunks.append(paras(body["bonus_paras"][:2]))
    chunks.append(figure(IMAGES["bonus"], body["img_bonus_alt"]))
    chunks.append(paras(body["bonus_paras"][2:]))
...
def main() -> None:
    ...
    for lang_id, meta in LOCALE_META.items():
        ...
        loc = {
            ...
            "url": meta.get("url", "1xbet-chicken-road"),
            "name": meta["name"],
            "title": meta["title"],
            ...
            "seo_monitor_ctx": {"entity": "casino_articles", "entity_id": 11},
        }
        new_locales.append(loc)

    cluster["entity"] = "casino_articles"
    cluster["entity_id"] = 11
