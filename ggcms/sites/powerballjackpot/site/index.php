...
// Missing static files must 404 here — not route through CMS (otherwise trailing-slash 301 breaks /file.js).
$__static_path = parse_url(isset($_SERVER['REQUEST_URI']) ? $_SERVER['REQUEST_URI'] : '', PHP_URL_PATH);
if ($__static_path === '/assets/images/chickenroad-hero.png') {
	header('HTTP/1.1 301 Moved Permanently');
	header('Location: /assets/images/chickenroad-hero.webp');
	exit;
}
if (is_string($__static_path) && strpos($__static_path, '/images/') === 0) {
	require(ROOT_DIR . 'images/index.php');
	die();
}
if (is_string($__static_path) && $__static_path !== ''
	&& preg_match('/\.(?:js|css|map|woff2?|ttf|eot|svg|png|jpe?g|gif|webp|ico|json|webmanifest)(?:\.gz)?$/i', $__static_path)
) {
	$__static_file = ROOT_DIR . ltrim($__static_path, '/');
	if (!is_file($__static_file)) {
...
require_once(ROOT_DIR.'functions/page_routing.php');
...
// pages#6 slug migration: /{lang}/predictor/ → /{lang}/strategies/
if (preg_match('#^/([a-z]{2,3})/predictor/?$#i', $norm_path, $pred_m)) {
	$qs = (isset($request_url[1]) && $request_url[1] !== '') ? '?' . $request_url[1] : '';
	header('HTTP/1.1 301 Moved Permanently');
	header('Location: /' . strtolower($pred_m[1]) . '/strategies/' . $qs);
	exit;
}
...
$abc['debug_ip_check'] = (isset($_GET['debug_ip_check']) && (string)$_GET['debug_ip_check'] === '1')
...
if ($go_code !== null && !empty($abc['advertising_api']['mode']) && $abc['advertising_api']['mode'] === 'api') {
	...
	$debug_ip_check = (isset($_GET['debug_ip_check']) && (string)$_GET['debug_ip_check'] === '1')
		|| !empty($config['debug_ip_check'])
		|| !empty($abc['advertising_api']['debug_ip_check']);
...
function page_i18n_slug($page_id, $lang_id) {
	...
	static $index_page_ids = null;
	if ($index_page_ids === null) {
		$index_page_ids = array();
		$idx_rows = mysql_select("SELECT id FROM pages WHERE module='index'", 'rows', 0);
		if ($idx_rows) {
			foreach ($idx_rows as $ir) {
				$index_page_ids[(int)$ir['id']] = true;
			}
		}
	}
	if (!empty($index_page_ids[$page_id])) {
		return null;
	}
...
//        elseif($u[1]=='cota2')		$where="module='cota2'";
			...
			$section_where = site_section_build_pages_slug_where($slug_raw, $langid);
			if ($section_where !== null) {
				$where = $section_where;
			} else {
			...
			if ($langid !== '' && $section_where === null) {
				static $url_langid_has_col_dbg = array();
				if (!array_key_exists($langid, $url_langid_has_col_dbg)) {
					$col = 'url' . $langid;
					$url_langid_has_col_dbg[$langid] = (mysql_select("SHOW COLUMNS FROM pages LIKE '" . mysql_res($col) . "'", 'num_rows') > 0);
				}
			}
			if ($abc['route_debug'] !== null) {
				if ($section_where !== null) {
					$abc['route_debug']['page_lookup'] = array(
						...
						'section_slug_variants' => site_section_slug_sql_variants($slug_raw),
					...
					$abc['route_debug']['page_lookup'] = array(
						...
						'pages_column_exists' => !empty($url_langid_has_col_dbg[$langid]),
					);
				}
			}
			if ($abc['route_debug'] !== null && $section_where === null && $langid === '') {
				$abc['route_debug']['page_lookup'] = array(
					'slug_from_path' => $slug_raw,
	...
	$route_debug_ci_sql = '';
	if ($error == 0 && !$abc['page'] && isset($slug_raw) && isset($where) && $where !== "module='index'" && @mysql_select("SHOW TABLES LIKE 'content_i18n'", 'num_rows') > 0) {
		$lid = (int)(isset($lang['id']) ? $lang['id'] : 0);
		if ($lid > 0) {
			$ci_slug_variants = site_section_slug_sql_variants($slug_raw);
			$url_ors = array();
			$ci_ors = array();
			foreach ($ci_slug_variants as $sv) {
				$esc = mysql_res($sv);
				$url_ors[] = "p.url='" . $esc . "'";
				$ci_ors[] = "ci.url='" . $esc . "'";
				$ci_ors[] = "ci.url='/" . $esc . "'";
	...
	if ($error == 0 AND $abc['page']) {
		site_section_legacy_redirect_if_needed($u, $lang);
		...
		// Guides page: use guides module when URL is guides or renamed slug (e.g. odds).
		if (site_page_is_section($abc['page'], 'guides', $langid)) {
		...
		// Casinos page: use casinos module when URL is casinos, lotteries, or legacy casinos slug.
		if (site_page_is_section($abc['page'], 'casinos', $langid)) {
			$abc['module'] = 'casinos';
...
*/
			...
			$abc['breadcrumb'][] = array(
				'name'=>(function() use ($abc, $langid, $lang) {
					...
					if ($slug === 'guides' || $slug === site_section_public_slug('guides')) return (string)i18n('common|guides_title');
					if ($slug === 'promo' || $slug === site_section_public_slug('promo')) {
