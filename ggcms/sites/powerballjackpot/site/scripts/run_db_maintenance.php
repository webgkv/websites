 ...
 *   php run_db_maintenance.php clean --target=blog --wipe-all --chunk=500 --max-chunks=500 --optimize
 *   php run_db_maintenance.php clean --target=blog --wipe-all --dry-run
 *
...
if ($is_cli) {
	...
	if ($args['mode'] === '') {
		...
		echo "      [--target=blog --wipe-all]  # delete ALL blog posts + content_i18n (all langs)\n";
		exit(0);
	}
} else {
	if (empty($_GET['run'])) {
...
if (!empty($args['wipe_all'])) {
	$wipe_ok = (count($targets) === 1 && isset($targets[0]) && in_array($targets[0], db_maintenance_wipe_all_targets(), true));
	if (!$wipe_ok) {
		$msg = '--wipe-all requires --target=translation_vector_items or --target=blog.';
		if ($is_cli) {
			fwrite(STDERR, $msg . "\n");
			exit(1);
		}
...
if ($args['mode'] === 'clean' && count($targets) === 1 && isset($targets[0]) && $targets[0] === 'blog' && empty($args['wipe_all'])) {
	$msg = 'Blog cleanup requires --wipe-all (removes all posts, categories, tags, and content_i18n for entity=blog).';
	if ($is_cli) {
		fwrite(STDERR, $msg . "\n");
		exit(1);
	}
