...
$form[0][] = array('text td12', '', array(
	...
	'value' =>
		...
		. '<div class="d-flex flex-wrap align-items-end gap-2 js-langpack-import" data-action="/admin.php?m=languages_json&u=import_pack' . ($lang_id_for_links > 0 ? '&lang_id=' . $lang_id_for_links : '') . '">'
		...
		. '</div>'
		. '<div class="mt-2"><a class="btn btn-sm btn-outline-primary" href="/admin.php?m=languages_json&u=export_pack">Export language pack (all languages)</a></div>'
		. ($lang_id_for_links > 0
			? ('<div class="mt-1"><a class="btn btn-sm btn-link p-0" href="/admin.php?m=languages_json&u=export_full&lang_id=' . $lang_id_for_links . '">Export this language only</a></div>')
			: ''
		)
