#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Apply sw/ln to Ice Fish casino_articles cluster JSON."""
...
from casino_sw_ln import get_fr_ln_pairs, get_meta, get_pairs, ln_from_fr  # noqa: E402
...
def en_template(data: dict) -> str:
    ...
    entity = str(data.get("entity") or "casino_articles")
...
def apply_cluster(data: dict) -> list[str]:
    ...
    for lang_id, lang in LANG_IDS.items():
        ...
        if not meta or not content or plain_len(content) < PLAIN_MIN:
            logs.append(f"casino#{eid} {lang}: missing/short")
            continue
        ...
        logs.append(f"casino#{eid} {lang}: {plain_len(content)} tags={bad or 'match'}")
    return logs
