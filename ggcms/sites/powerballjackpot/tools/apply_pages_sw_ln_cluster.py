#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Apply Swahili (20) and Lingala (21) to PowerBall Jackpot pages cluster JSON export."""
...
import pages_content_sw_ln as cr_pages  # noqa: E402
...
def _cr_content(entity_id: int, lang: str) -> str | None:
    content = cr_pages.get_content(entity_id, lang)
    if not content:
        return None
    content = localize_hrefs(content, lang)
    return wrap_internal_links_noads(content)
...
def build_sw_content(data: dict) -> str | None:
    ...
    return _cr_content(entity_id, "sw")
...
def build_ln_content(data: dict) -> str | None:
    ...
    if ln_from_fr(entity_id):
        ...
        if pairs:
            ...
            html = localize_hrefs(html, "ln")
            return wrap_internal_links_noads(html)
    return _cr_content(entity_id, "ln")
...
def resolve_meta(entity_id: int, lang: str) -> dict[str, str] | None:
    ...
    if meta:
        return meta
    cr_meta = cr_pages.get_meta_override(entity_id, lang)
    return cr_meta
...
def apply_cluster(data: dict) -> list[str]:
    entity_id = int(data.get("entity_id") or 0)
    logs: list[str] = []
    if entity_id == 1 and is_hub_only(entity_id):
        for loc in data["locales"]:
            lead = (loc.get("description") or loc.get("title") or "").strip()
            if lead:
                loc["content"] = (
                    '<div class="about_content page-content-lead"><p>'
                    + html_mod.escape(lead, quote=False)
                    + "</p></div>"
                )
            else:
                loc["content"] = ""
        logs.append("pages#1 all locales: meta-only lead paragraph (no H1 in body)")
    ...
    for lang_id, lang in LANG_IDS.items():
        ...
        if not meta:
            ...
            continue
        if entity_id == 1 and is_hub_only(entity_id):
            lead = (meta.get("description") or meta.get("title") or "").strip()
            content = (
                '<div class="about_content page-content-lead"><p>'
                + html_mod.escape(lead, quote=False)
                + "</p></div>"
