#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Apply sw/ln to Ice Fish games cluster JSON."""
...
sys.path.insert(0, str(CHICKEN))  # games_i18n_utils
sys.path.insert(0, str(TOOLS))  # games_sw_ln (this site’s data dir)
...
from games_sw_ln import get_fr_ln_pairs, get_meta, get_pairs, ln_from_fr  # noqa: E402
...
def build_sw(data: dict) -> str | None:
    ...
    if not pairs:
        return None
    html = wrap_internal_links_noads(localize_hrefs(apply_pairs(en_template(data), pairs), "sw"))
    return html
...
def apply_cluster(data: dict) -> list[str]:
    ...
    for lang_id, lang in LANG_IDS.items():
        ...
        if not meta or not content or plain_len(content) < PLAIN_MIN:
            logs.append(f"games#{eid} {lang}: missing/short")
            continue
        ...
        logs.append(f"games#{eid} {lang}: {plain_len(content)} tags={bad or 'match'}")
    return logs
