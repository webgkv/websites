#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Apply sw/ln to Ice Fish guides cluster JSON."""
...
from guides_sw_ln import get_fr_ln_pairs, get_meta, get_pairs, ln_from_fr  # noqa: E402
...
def build_sw(data: dict) -> str | None:
    ...
    return wrap_internal_links_noads(localize_hrefs(html, "sw"))


def build_ln(data: dict) -> str | None:
    eid = int(data.get("entity_id") or 0)
    if ln_from_fr(eid):
        pairs = get_fr_ln_pairs(eid)
        fr = fr_template(data)
        if not pairs or not fr:
            return None
        html = apply_pairs(fr, pairs)
    else:
        pairs = get_pairs(eid, "ln")
        if not pairs:
            return None
        html = apply_pairs(en_template(data), pairs)
    return wrap_internal_links_noads(localize_hrefs(html, "ln"))
...
def apply_cluster(data: dict) -> list[str]:
    ...
    for lang_id, lang in LANG_IDS.items():
        ...
        if not meta or not content or plain_len(content) < PLAIN_MIN:
            logs.append(f"guides#{eid} {lang}: missing/short")
            continue
        ...
        logs.append(f"guides#{eid} {lang}: {plain_len(content)} tags={bad or 'match'}")
    return logs
