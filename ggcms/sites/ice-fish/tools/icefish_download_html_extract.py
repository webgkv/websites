...
HTML_PATH = Path(__file__).resolve().parents[1] / "tmp/jason/Ice Fish - download/ChickenRoaddownload.html"
...
def extract_english(html: str | None = None) -> dict:
    ...
    intro_paras = _section_paras(
        html,
        r"Ice Fish app download: how to access the game on Android, iPhone and PC</span></p>",
        r"What Is the Ice Fish Application",
    )

    what_h2 = "What Is the Ice Fish Application?"
    what_first = _clean(
        re.search(
            r"What Is the Ice Fish Application\?<br></span><span class=\"c1\">(.*?)</span></p>",
            html,
            re.S,
        ).group(1)
    )
    what_rest = _section_paras(
        html,
        r"What Is the Ice Fish Application\?<br></span><span class=\"c1\">",
        r"<table class=\"c17\">",
    )
    ...
    req_h2 = "The Ice Fish APP System Requirements"
    req_table = [(r[0], r[1], r[2]) for r in tables[1][1:] if len(r) == 3]

    download_h2 = "How to Download the Ice Fish Game App"
    download_paras = _section_paras(
        html,
        r"How to Download the Ice Fish Game App</span></p>",
    ...
    use_h2 = "How to Use the Ice Fish Game App"
    use_paras = _section_paras(
        html,
        r"How to Use the Ice Fish Game App</span></p>",
