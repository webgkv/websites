...
<?php
/**
 * Rename legacy Aviator guide page slugs on ice-fish.run and rebrand stored copy.
 *
...
function mig_rebrand_fields(array $row, array $fields) {
	...
	foreach ($fields as $f) {
		...
		$new = icefish_normalize_legacy_slug_urls_in_text($new);
		if ($new !== $orig) {
			$patch[$f] = $new;
		}
	}
	...
	if (@mysql_select("SHOW TABLES LIKE 'content_i18n'", 'num_rows') > 0) {
		...
		foreach ($rows as $r) {
			...
			if ($url === $old_slug || $url === 'guides/' . $old_slug) {
				...
				$ci_patch['url'] = icefish_normalize_legacy_slug_urls_in_text($url);
				if ($ci_patch['url'] !== $url) {
					$report['content_i18n_url']++;
				}
			}
...
if (@mysql_select("SHOW TABLES LIKE 'content_i18n'", 'num_rows') > 0) {
	...
	foreach ($all as $r) {
		...
		$new_url = icefish_normalize_legacy_slug_urls_in_text($url);
		if ($new_url !== $url) {
