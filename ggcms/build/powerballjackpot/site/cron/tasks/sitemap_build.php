...
$casinos_page = mysql_select("
	SELECT * FROM pages
	WHERE display=1
	  AND (
		module='casinos'
		OR (module='pages' AND " . sm_pages_slug_where('casinos') . ")
		OR (module='pages' AND " . sm_pages_slug_where('lotteries') . ")
	  )
	LIMIT 1
", 'row');
...
foreach ($languages as $lang) {
	...
	// 5b) Promo
	if (!$seo_index_whitelist && !empty($include['promo']) && @mysql_select("SHOW TABLES LIKE 'promo'", 'num_rows') > 0) {
