...
foreach ($menu as $k=>$v) {
	...
	// Strategies (pages#6) menu fallback when content_i18n name is empty.
	...
	if (trim((string)($v["name$langid"] ?? '')) === '' && in_array($base_slug, array('strategies', 'predictor'), true)) {
		$lid = (int)$langid;
		$dict_common = ROOT_DIR . 'files/languages/' . $lid . '/dictionary/common.php';
		if ($lid > 0 && is_file($dict_common)) {
			$fb = (string)i18n('common|strategies_menu');
			if (trim($fb) === '') {
				$fb = (string)i18n('common|predictor_menu');
			}
			if (trim($fb) !== '') $v["name$langid"] = $fb;
		}
		if (trim((string)($v["name$langid"] ?? '')) === '') $v["name$langid"] = $base_name !== '' ? $base_name : 'Strategies';
	}
	...
	} elseif (isset($v['module']) && (string)$v['module'] === 'index') {
		$v['_url'] = get_url('index');
	} else {
...
// Optional: place Articles (blog) between Results and Odds (guides) in menu.
$menu_order = array_values(array_keys($abc['menu']));
$blog_id = null;
$results_id = null;
$guides_id = null;
foreach ($abc['menu'] as $id => $item) {
	if (isset($item['module']) && $item['module'] === 'blog') {
		$blog_id = $id;
	}
	if (site_menu_item_matches_section($item, 'guides')) {
		$guides_id = $id;
	}
	if (site_page_menu_url_slug($item) === 'results') {
		$results_id = $id;
	}
}
if ($blog_id !== null && $results_id !== null && $guides_id !== null) {
	$pos_blog = array_search($blog_id, $menu_order, true);
	$pos_results = array_search($results_id, $menu_order, true);
	$pos_guides = array_search($guides_id, $menu_order, true);
	if ($pos_blog !== false && $pos_results !== false && $pos_guides !== false && $pos_blog !== $pos_results + 1) {
		array_splice($menu_order, $pos_blog, 1);
		$pos_guides = array_search($guides_id, $menu_order, true);
		if ($pos_guides !== false) {
			array_splice($menu_order, $pos_guides, 0, array($blog_id));
		}
		$new_menu = array();
		foreach ($menu_order as $mid) {
			if (isset($abc['menu'][$mid])) {
				$new_menu[$mid] = $abc['menu'][$mid];
			}
		}
		$abc['menu'] = $new_menu;
	}
...
if (@mysql_select("SHOW TABLES LIKE 'variables'", 'num_rows') > 0) {
	...
	if (function_exists('site_seo_normalize_structured')) {
		site_seo_normalize_structured($abc['seo_structured']);
	}
}
