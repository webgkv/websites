...
if (!defined('AVIATOR_APK_FILE_BASENAME')) {
	define('AVIATOR_APK_FILE_BASENAME', function_exists('site_brand_apk_basename') ? site_brand_apk_basename() : 'powerballjackpot.apk');
}
...
function aviator_apk_normalize_apk_link_in_content($html) {
	...
	$legacy = array('aviator_v1.apk', 'aviator.apk');
 ...
 * DB exports often ship with a fixed ?v=1, which keeps stale images in the browser.
 *
 * @param string $html
 * @return string
 */
function aviator_apk_bust_install_android_image_cache($html) {
	if ($html === '' || strpos($html, 'apk-install-android') === false) {
		return $html;
	}
	$names = array(
		'step-1-download-apk.png',
		'step-2-unknown-sources.png',
		'step-3-install-open.png',
	);
	$v = 0;
	if (defined('ROOT_DIR')) {
		foreach ($names as $name) {
			$f = ROOT_DIR . 'assets/images/apk-install-android/' . $name;
			if (@file_exists($f)) {
				$v = max($v, (int) filemtime($f));
			}
		}
...
function aviator_apk_seo_cluster_content_html(array $b) {
	...
	$out = '<h1>' . $b['h1'] . '</h1>' . $sep
		...
		. '<p class="mt-3"><a class="cmn-btn" href="' . htmlspecialchars($apk, ENT_QUOTES, 'UTF-8') . '" download="' . htmlspecialchars(AVIATOR_APK_FILE_BASENAME, ENT_QUOTES, 'UTF-8') . '">'
