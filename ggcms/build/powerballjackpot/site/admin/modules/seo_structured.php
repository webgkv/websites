...
if (!$variables_exists) {
	...
	$content .= '<input type="text" class="form-control" name="seo_canonical_base" id="seo_canonical_base" value="' . htmlspecialchars($seo['canonical_base']) . '" placeholder="https://powerballjackpot.run" /></div>';
	$content .= '<p class="small text-muted col-12">Must match this site’s public host (not a legacy domain). Preset: <code>site/files/reference/seo_structured-powerballjackpot-preset.json</code></p>';
	$content .= '<div class="form-group col-md-6"><label class="form-label" for="seo_site_name">Site name</label>';
	$content .= '<input type="text" class="form-control" name="seo_site_name" id="seo_site_name" value="' . htmlspecialchars($seo['site_name']) . '" placeholder="PowerBall Jackpot" /></div>';
	$content .= '</div></div></div>';
