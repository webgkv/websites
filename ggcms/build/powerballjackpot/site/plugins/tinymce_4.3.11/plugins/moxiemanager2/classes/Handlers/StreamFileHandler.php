...
class MOXMAN_Handlers_StreamFileHandler implements MOXMAN_Http_IHandler {
	...
	public function processRequest(MOXMAN_Http_Context $httpContext) {
		...
		$tempname = MOXMAN_Util_Sanitize::fileName($request->get("tempname"));
		...
		try {
			...
			$response->setStatus("500", "Could not resolve path: " . $path);

			if (MOXMAN::getLogger()) {
				MOXMAN::getLogger()->debug("Could not resolve path: " . $path);
			}
		...
		if ($tempName = MOXMAN_Util_Sanitize::fileName($tempname)) {
