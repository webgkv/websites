...
<?php } ?>
                            <li class="nav-item<?= !empty($q['_submenu']) ? ' dropdown' : '' ?>">
                                ...
                                <?php else : ?>
                                <?php
                                $is_menu_cta = ($i == $num_rows && empty($q['_submenu']));
                                $nav_classes = 'nav-link' . (!empty($q['_active']) ? ' active' : '') . ($is_menu_cta ? ' cmn-btn' : '');
                                ?>
                                <a class="<?= $nav_classes ?>" href="<?= str_replace('//','/', $q['_url']) ?>"><?= htmlspecialchars($q['name']) ?></a>
                                <?php endif; ?>
                            </li>
<?php if($i==$num_rows) { ?>
                        </ul>
