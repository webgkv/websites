...
<?php
/**
 * Full Aviator → Chicken Road cleanup on chickenroad.run (DB + dictionary files).
 *
...
function mig_rebrand_table($table, array $text_fields, array &$report) {
	...
	foreach ($rows as $row) {
		...
		if (isset($cols['url']) && isset($row['url']) && stripos((string) $row['url'], 'aviator') !== false && $table === 'casino_articles') {
			$patch['url'] = chickenroad_casino_slug_to_chickenroad($row['url']);
		}
		if (empty($patch)) {
			continue;
		}
		...
		if ($url !== '' && stripos($url, 'aviator') !== false) {
			if (($r['entity'] ?? '') === 'casino_articles' && !empty($r['entity_id'])) {
				...
				if ($base && !empty($base['url'])) {
					...
					$patch['url'] = chickenroad_casino_slug_to_chickenroad($url);
				}
			} else {
				$patch['url'] = chickenroad_normalize_legacy_slug_urls_in_text($url);
...
if (@mysql_select("SHOW TABLES LIKE 'redirects'", 'num_rows') > 0) {
	...
	foreach ($legacy_casino as $old_slug) {
		$candidate = chickenroad_casino_slug_to_chickenroad($old_slug);
		foreach ($lang_segs as $lang) {
			$prefix = ($lang !== '' ? '/' . $lang : '');
			$new_path = ($candidate !== '' && chickenroad_casino_article_exists($candidate))
