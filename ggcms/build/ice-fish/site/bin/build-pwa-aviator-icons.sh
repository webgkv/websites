#!/usr/bin/env bash
# Build PWA icons from Ice Fish game tile (square cover crop, like Aviator).
set -euo pipefail
...
if [[ -n "${1:-}" ]]; then
	SRC="$1"
elif [[ -f "$IMG/ice-fish-pwa-source.png" ]]; then
	SRC="$IMG/ice-fish-pwa-source.png"
elif [[ -f "$IMG/ice-fish-pwa-source.webp" ]]; then
	SRC="$IMG/ice-fish-pwa-source.webp"
elif [[ -f "$IMG/ice-fish-hero.webp" ]]; then
	SRC="$IMG/ice-fish-hero.webp"
elif [[ -f "$IMG/ice-fish-hero.png" ]]; then
	SRC="$IMG/ice-fish-hero.png"
else
	echo "Missing source image. Save one as:" >&2
	echo "  $IMG/ice-fish-pwa-source.png" >&2
