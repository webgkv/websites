...
if (!$variables_exists) {
	...
	$content .= '<input type="text" class="form-control" name="seo_canonical_base" id="seo_canonical_base" value="' . htmlspecialchars($seo['canonical_base']) . '" placeholder="https://ice-fish.run" /></div>';
	$content .= '<p class="small text-muted col-12">Must match this site’s public host (not a legacy Aviator domain). Preset: <code>site/files/reference/seo_structured-ice-fish-preset.json</code></p>';
	$content .= '<div class="form-group col-md-6"><label class="form-label" for="seo_site_name">Site name</label>';
	$content .= '<input type="text" class="form-control" name="seo_site_name" id="seo_site_name" value="' . htmlspecialchars($seo['site_name']) . '" placeholder="Ice Fish" /></div>';
	$content .= '</div></div></div>';
