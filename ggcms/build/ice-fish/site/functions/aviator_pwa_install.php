 ...
 */
function aviator_pwa_demo_app_absolute_url($abc, $lang) {
	$path = aviator_demo_app_shell_url($abc, $lang);
	if (function_exists('aviator_seo_public_origin')) {
		$origin = aviator_seo_public_origin();
		if ($origin !== '') {
 ...
 * @return string
 */
function aviator_pwa_bust_install_ios_image_cache($html) {
	if ($html === '' || strpos($html, 'pwa-install-ios') === false) {
		return $html;
	}
	$names = array(
		'step-1-share-sheet.jpg',
		'step-2-add-to-home-screen.jpg',
		'step-3-open-as-web-app.jpg',
	);
	$v = 0;
	if (defined('ROOT_DIR')) {
...
function aviator_pwa_normalize_demo_links_in_content($html, $abc, $lang) {
	if ($html === '' || stripos($html, 'demo/app') === false) {
		return $html;
	}
	$url = aviator_pwa_demo_app_absolute_url($abc, $lang);
...
function aviator_pwa_seo_cluster_content_html(array $b) {
	$imgV = 0;
	$names = array('step-1-share-sheet.jpg', 'step-2-add-to-home-screen.jpg', 'step-3-open-as-web-app.jpg');
	if (defined('ROOT_DIR')) {
		foreach ($names as $name) {
			$f = ROOT_DIR . 'assets/images/pwa-install-ios/' . $name;
			if (@file_exists($f)) {
