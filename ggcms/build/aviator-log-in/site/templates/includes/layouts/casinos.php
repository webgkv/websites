<?php
// Casinos: list (9 cards + pagination) or single article
$langid = isset($abc['langid']) ? $abc['langid'] : '';
$lang_prefix = isset($abc['lang']['url']) ? trim((string)$abc['lang']['url'], '/') : '';
// Build base link explicitly to avoid missing `/casinos/` segment and double slashes.
$casinos_base = preg_replace('#/+#', '/', ($lang_prefix !== '' ? '/' . $lang_prefix . '/casinos/' : '/casinos/'));
...
<section class="py-5">
    <div class="container">
        <?php if (!empty($abc['casino_single'])) : ?>
            ...
            <?php if (function_exists('aviator_render_author_byline')) echo aviator_render_author_byline($abc, array('date' => $article['date'] ?? ($article['updated_at'] ?? ''))); ?>
                        ...
                        <a href="<?= $link ?>" class="guide-card card h-100 text-decoration-none">
                            ...
                            <div class="card-body">
                                ...
                                <?php if (!empty($c['name_2'])) : ?>
                                    <p class="card-text guide-card-desc"><?= htmlspecialchars($c['name_2']) ?></p>
                                <?php endif; ?>
