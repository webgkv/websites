...
// Build base link explicitly to always include `/casinos/` and avoid double slashes.
		...
		<?php elseif (!empty($abc['casino_list']) || !empty($abc['casino_pagination'])) : ?>
			...
			<div class="row">
				...
				foreach ($list as $c) :
					...
					$img_src = (!empty($c['img']) && strpos((string)$c['img'], '/') === false)
						? '/images/casinos/' . htmlspecialchars((string)$c['img'])
						: '';
				?>
