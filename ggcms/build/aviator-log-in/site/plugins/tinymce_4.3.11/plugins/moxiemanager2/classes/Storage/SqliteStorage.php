		...
		switch ($this->type) {
			case self::TYPE_GLOBAL:
				if ($pdo->i('SELECT COUNT(pr_id) FROM moxman_property WHERE pr_user IS NULL AND pr_group IS NULL AND pr_name = ?', $name) > 0) {
					$pdo->q("UPDATE moxman_property SET pr_value = :value, pr_modificationdate = DATETIME('NOW') WHERE pr_user IS NULL AND pr_group IS NULL AND pr_name = :name", array(
						"name" => $name,
						"value" => $value
					));
				} else {
					$pdo->q("INSERT INTO moxman_property(pr_name, pr_value, pr_creationdate, pr_modificationdate) VALUES(:name, :value, DATETIME('NOW'), DATETIME('NOW'))", array(
						"name" => $name,
						"value" => $value
					));
				}
			...
			case self::TYPE_GROUP:
				if ($pdo->i('SELECT COUNT(pr_id) FROM moxman_property WHERE pr_group = ? AND pr_name = ?', $this->name, $name) > 0) {
					$pdo->q("UPDATE moxman_property SET pr_value = :value, pr_modificationdate = DATETIME('NOW') WHERE pr_group = :grname AND pr_name = :name", array(
						"grname" => $this->name,
						"name" => $name,
						"value" => $value
					));
				} else {
					$pdo->q("INSERT INTO moxman_property(pr_group, pr_name, pr_value, pr_creationdate, pr_modificationdate) VALUES(:grname, :name, :value, DATETIME('NOW'), DATETIME('NOW'))", array(
						"grname" => $this->name,
						"name" => $name,
						"value" => $value
					));
			...
			case self::TYPE_USER:
				if ($pdo->i('SELECT COUNT(pr_id) FROM moxman_property WHERE pr_user = ? AND pr_name = ?', $this->name, $name) > 0) {
					$pdo->q("UPDATE moxman_property SET pr_value = :value, pr_modificationdate = DATETIME('NOW') WHERE pr_user = :usname AND pr_name = :name", array(
						"usname" => $this->name,
						"name" => $name,
						"value" => $value
					));
				} else {
					$pdo->q("INSERT INTO moxman_property(pr_user, pr_name, pr_value, pr_creationdate, pr_modificationdate) VALUES(:usname, :name, :value, DATETIME('NOW'), DATETIME('NOW'))", array(
						"usname" => $this->name,
						"name" => $name,
						"value" => $value
					));
