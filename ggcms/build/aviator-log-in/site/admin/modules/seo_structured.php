...
if (!$variables_exists) {
	...
	$content .= '<input type="text" class="form-control" name="seo_canonical_base" id="seo_canonical_base" value="' . htmlspecialchars($seo['canonical_base']) . '" placeholder="https://example.com" /></div>';
	$content .= '<div class="form-group col-md-6"><label class="form-label" for="seo_site_name">Site name</label>';
	$content .= '<input type="text" class="form-control" name="seo_site_name" id="seo_site_name" value="' . htmlspecialchars($seo['site_name']) . '" placeholder="Site title for WebPage / breadcrumbs" /></div>';
	$content .= '</div></div></div>';
